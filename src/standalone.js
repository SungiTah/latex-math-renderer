// Standalone LaTeX Renderer - No modules required
class LaTeXRenderer {
  constructor() {
  ...
  renderMath() {
    ...
    try {
        ...
        this.mathOutput.innerHTML = `
          <div class="text-center p-4">
            <div class="bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
              ...
              <p class="text-yellow-700 dark:text-yellow-300 font-medium">KaTeX Loading...</p>
              <p class="text-yellow-600 dark:text-yellow-400 text-sm mt-2">Please wait for KaTeX to load completely.</p>
            </div>
          </div>
        `
        return
      ...
      this.mathOutput.innerHTML = `
        <div class="math-container p-4 text-center">
          ${html}
        </div>
      `
  ...
  async exportAsImage(format) {
    ...
    try {
      ...
      if (typeof html2canvas === 'undefined') {
        this.showToast('Export functionality is loading...', 'warning')
        return
      }
  ...
  renderHistory() {
    ...
    const historyHTML = this.history.map(item => `
      <div class="history-item group p-3 border border-gray-200 dark:border-gray-600 rounded-lg cursor-pointer transition-all duration-200 hover:border-blue-300 dark:hover:border-blue-500 hover:shadow-md bg-gray-50 dark:bg-gray-700/50 mb-3" data-latex="${this.escapeHtml(item.latex)}" data-id="${item.id}">
...
Try typing any mathematical expression!`
    ...
    window.latexApp = app
  }, 1000) // Wait 1 second for KaTeX and html2canvas to load
})